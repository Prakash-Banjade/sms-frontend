...
const ExamTypeSelectById = ({ data, placeholder, label, onSelectChange }: ExamTypeSelectProps) => {
    const [searchParams, setSearchParams] = useSearchParams();

    // Initialize the selected ExamType number from URL, if it exists
    const initialExamTypeNumber = searchParams.get("examTypeId") || '';
    ...
    useEffect(() => {
        if (selectedExamType) {
            setSearchParams({ examTypeId: selectedExamType });
        }
...
export default ExamTypeSelectById;
