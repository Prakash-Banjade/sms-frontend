...
export function DynamicMultiSelect<T extends FieldValues, F = any>({
    ...
    const handleOnClear = () => {
        setFormValue(name as string, undefined)
                        ...
                        <PopoverTrigger asChild className="hover:bg-secondary/20">
                            ...
                            >
                                <div className="flex gap-2 justify-start">
                                    {value?.length ?
                                        value.map((val, i) => (
                                            <div key={i} className="px-2 py-1 rounded-xl border bg-secondary text-xs font-medium">{options.find((option) => option.value === val)?.label}</div>
                                        ))
                                        : placeholder}
                                </div>
